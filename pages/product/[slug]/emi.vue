<template>
    <div>
        <main v-if="product != null" class="main pages">
        <div class="page-header breadcrumb-wrap">
            <div class="container">
                <div class="breadcrumb">
                    <a href="/" rel="nofollow"><i class="fi-rs-home mr-5"></i>Home</a>
                    <span></span> Product <span></span> {{ product.p_name }} <span></span> <i class="fa-solid fa-indian-rupee-sign"></i> EMI
                </div>
            </div>
        </div>
        <div class=" emi" style="padding: 50px 70px;">
            <div class="row">
				<div id="content" class="col-sm-12 px-5">
					<h2 class="title"><i class="fa fa-indian-rupee-sign" style="color: orange;"></i> EMI <span class="h4">({{ product.p_name }})</span></h2>
					<br />
					<form action="javascript:void(0)" @submit="doSubmit" enctype="multipart/form-data"
							...
							<div class="col-12 px-3">
								...
								<div class="row">
									<div class="required col-sm-6">
										...
										<input type="text"  name="product_price" disabled placeholder=""
											class="form-control" :value="`Rs.${product.p_sale_price}/-`">
									</div>
                                    <div class="required col-sm-6">
							...
							<div class="col-12 px-3">
								<div class="row">
									...
									<div class="required col-sm-6">
										...
										<input type="text" name="Pin Code" @change="validatePinCode()"  v-model="ProfessionDetails.pinCode" placeholder="Pin Code" class="form-control">
						...
						<div class="buttons px-5">
							 <div class="pull-right d-flex " style="justify-content: space-between;">
                                ...
                                <button @submit="doSubmit()" class="btn">Submit</button>
							</div>
						</div>
					</form>
				</div>
...
export default {
	data() {
		return {
            ...
            availableService: ["313327","313324","313328","311806","313329"],

		}
	},
    async created() {
        console.log('Mounted Emi');
        await this.init();
    },
	methods: {
     async init() {
     ...
     async doSubmit() {
			...
			formData.append('regionState', this.regionState);
				...
				const res = await axios.get(apiUrl.addEmiInformation, formData);
				console.log(res);
        ...
        async validatePinCode() {
            ...
            const res = await axios.get('https://api.postalpincode.in/pincode/'+this.ProfessionDetails.pinCode);

            if(this.ProfessionDetails.pinCode.length == 6) {
                ...
                if(res.status == '200') {
                    ...
                    if(res.data[0].status == 'Error') {
                        ...
                        this.ProfessionDetails.city = res.data[0].PostOffice[0].District;
						this.ProfessionDetails.regionState = res.data[0].PostOffice[0].State; 

                        var isServiceAvailable = false;

                        this.availableService.forEach((pincode) => {
							if(pincode == this.ProfessionDetails.pinCode) {
                        ...
                        {
                            alert('service is not available on this pincode');
                        }
                    }
